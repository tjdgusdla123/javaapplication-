...
public class ItemController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//문제 발생시 1번 확인
		//컨트롤러1,2번 확인하고 itemservicelmpl에서 3,4번 확인하고  itemDao에서 5,6,7번  문제해결하기 
		//요청이 여기까지 도달하는지 확인
		//여기에 도달하지 않으면 Controller 처리 패턴이 잘못었거나 링크 설정을 잘못한 것입니다.
		//command를 아래 제어문에서 처리하는지 확인
		//command와 아래 처리하지 않는 URL이 일치하지 않으면 처리가 안됨
		//일치하지 않으면 요청 페이지의 URL을 변경하던지 처리구문의 URL을 변경하던지 해야 합니다.
		...
		if (command.equals("/")) {
			...
			//문제 발생시 2번 확인
			//여기까지 오는지 확인 - 1번은 출력되고 이것은 출력안되면 요청 페이지의 요청 URL 과 비교하는 URL 수정
			System.out.println("2:"+command);
			
			//상세보기를 처리
			itemService.detail(request, response);
			//결과 페이지로이동
			//결과 페이지에 jsp 파일이 제데로 생성되었는지 확인		
